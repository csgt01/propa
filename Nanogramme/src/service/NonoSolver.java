...
public class NonoSolver implements INonogramSolver {
	 ...
	 * {@link #changeLastStacksMember()} aufgerufen oder die Methode
	 * abgebrochen.
	 * 
	 * 
	 */
	 ...
	 */
	protected boolean checkStateOfWrittenMatrixByRow(char[][] checkMatrix,
			int rowIn) {
				...
				if (checkMatrix[rowInt][columnInt] == empty) {
					...
					if (blockInt >= blocks.size()) {
						...
						for (int i = 0; i < block.getHowMany(); i++) {
							if (!(checkMatrix[rowInt][columnInt] == block
									.getColorChar())) {
								System.out.println("false");
								return false;
							}
	 ...
	 * @return true, falls in Reihe und Spalte die Farbe in einem Block
	 *         vorkimmt.
	 */
	protected boolean isColorInThisRowAndColumn(int rowInt, int columnInt,
			int indexOfColor) {
		Row row = getRows().get(rowInt);
		boolean isPresent = false;
		int index = 0;
		...
		while (index < size && !isPresent) {
			...
			if (block.getColorChar() == riddle.getColours().get(indexOfColor)
					.getName()) {
	 ...
	 * der Stackholder entfernt und diese Methode nochmal aufgerufen, um den
	 * vorherigen Stackholder zu ändern.
	 * 
			...
			if (indexOfColor < -1) {
				...
				try {
					if (indexOfColor > -1) {
						...
						writeCharInMatrix(lastStackHolder.getRow(), '-',
								lastStackHolder.getColumn());
					}
				} catch (Exception e) {
					e.printStackTrace();
	 ...
	 * Überprüft nun auch, ob es die Farbe, die geraten werden soll, in der
	 * Reihe und Spalte gibt.
	 * 
	 * @throws Exception
		...
		for (int row = 0; row < riddle.getHeight(); row++) {
			for (int column = 0; column < riddle.getWidth(); column++) {
				if (matrix[row][column] == '*') {
					...
					// int colorIndex = (-1);
					while (colorIndex > -1
							&& !isColorInThisRowAndColumn(row, column,
									colorIndex)) {
						colorIndex--;
					}
					...
					if (colorIndex != -1) {
						writeCharInMatrix(
								row,
								riddle.getColours()
										.get(stack.getIndexOfColor()).getName(),
								column);
					} else {
				...
				if (blockInts.size() == 1) {
					...
					if ((row.getMaxEntries() - row.getEntriesSet() - starCount) == 0) {
						...
						fillAreaInRowWithChar(rowInt, columnInt, columnInt + 1,
								blocks.get(blockInts.get(0)).getColorChar());
					}
				...
				if (!block.isGone() && block.doOverlapping) {
					...
					while (workingList.getLast().equals("-")) {
						...
						for (int i = size - 1; i > -1; i--) {
							...
							if (!first.get(index2).equals(
									workingList.get(index2))) {
	 ...
	 * @throws Exception
	 */
	private void fillBlocksOnEndOfColumns() throws Exception {
		...
		for (int index = 0; index < size; index++) {
			Column column = getColumns().get(index);
			if (!column.isGone()) {
				fillBlocksOnEndOfColumn(column, index, column.getBlocks()
						.size() - 1, riddle.getHeight() - 1);
			}
		}
		// // // System.out.println("Time for " + methodName + ": " + (new
		// Date().getTime() - startTime) + " ms");
	 ...
	 * @throws Exception
	 */
	private void fillBlocksOnEndOfColumn(Column column, int indexOfColumn,
			int blockIndex, int rowIndex) throws Exception {
		...
		if (rowInt > -1 && matrix[rowInt][indexOfColumn] != '*') {
			...
			if (blocks != null) {
				...
				if (matrix[rowInt][indexOfColumn] == colourBlock.getColorChar()) {
					fillAreaInColumnWithChar(indexOfColumn, (rowInt
							- colourBlock.getHowMany() + 1), (rowInt + 1),
							colourBlock.getColorChar());
					...
					if (-1 < block - 1) {
						...
						if (nextBlock != null
								&& nextBlock.getColorChar() == colourBlock
										.getColorChar() && rowInt > -1) {
							fillAreaInColumnWithChar(indexOfColumn, rowInt,
									rowInt + 1, '-');
							rowInt--;
						}
						// nochmal aufrufen
						fillBlocksOnEndOfColumn(column, indexOfColumn, block,
								rowInt);
					} else {
	 ...
	 * @throws Exception
	 */
	private void fillBlocksOnBeginningOfColumns() throws Exception {
	 ...
	 * @throws Exception
	 */
	private void fillBlocksOnBeginningOfColumn(Column column,
			int indexOfColumn, int blockIndex, int rowIndex) throws Exception {
			...
			if (rowInt < riddle.getHeight()
				...
				if (blocks != null) {
					...
					if ((rowInt + howMany) < riddle.getHeight()
							&& matrix[rowInt][indexOfColumn] == colourBlock
									.getColorChar()) {
						fillAreaInColumnWithChar(indexOfColumn, rowInt,
								(rowInt + howMany), colourBlock.getColorChar());
						...
						if (blocks.size() > block + 1) {
							...
							fillBlocksOnBeginningOfColumn(column,
									indexOfColumn, block, rowInt);
	 ...
	 * @throws Exception
	 */
	private void fillBlocksOnEndOfRows() throws Exception {
		...
		if (columnInt > -1 && matrix[indexOfRow][columnInt] != '*') {
			...
			if (blocks != null) {
				...
				if (matrix[indexOfRow][columnInt] == colourBlock.getColorChar()) {
					fillAreaInRowWithChar(indexOfRow,
							(columnInt - colourBlock.getHowMany() + 1),
							(columnInt + 1), colourBlock.getColorChar());
					...
					if (-1 < block - 1) {
						...
						if (nextBlock != null
								&& nextBlock.getColorChar() == colourBlock
										.getColorChar()) {
							fillAreaInRowWithChar(indexOfRow, columnInt,
									columnInt + 1, '-');
							columnInt--;
						}
					...
					throw new DataCollisionException("char "
							+ matrix[indexOfRow][columnInt] + " ungleich "
							+ colourBlock.getColorChar());
				}
	 ...
	 * @throws Exception
	 */
	private void fillBlocksOnBeginningOfRows() throws Exception {
			...
			if (columnInt < riddle.getWidth()
				...
				if (blocks != null) {
					...
					if ((columnInt + colourBlock.getHowMany()) <= riddle
							.getWidth()
							&& matrix[indexOfRow][columnInt] == colourBlock
									.getColorChar()) {
						fillAreaInRowWithChar(indexOfRow, columnInt,
						...
						if ((columnInt + colourBlock.getHowMany()) < riddle
							...
							solveState = SolveStateEnum.ERROR;
							throw new DataCollisionException("char "
									+ matrix[columnInt][indexOfRow] + " at"
									+ columnInt + "/" + indexOfRow
									+ " ungleich " + colourBlock.getColorChar());
						}
					}
	 ...
	 * @param lastBlock
	 * @param resultIndex
	 * @return List<String> mit der Startbelegung der Blöcke
	 */
	private LinkedList<String> getFirstConditionOfRow(ArrayList<Block> blocks,
		...
		for (int index = 0; index < blocks.size(); index++) {
			Block block = blocks.get(index);
			if (null != lastBlock
					&& lastBlock.getColorChar() == block.getColorChar()) {
				asd.add("-");
				resultIndex++;
			}
	...
	private int getMinStartIndexOfBlock(ArrayList<Block> blocks,
		...
		for (int i = indexOfBlock - 1; i >= 0; i--) {
			...
			if (null != lastBlock
					&& lastBlock.getColorChar() == block.getColour().getName()) {
				index++;
			}
			lastBlock = block;
		}
	 ...
	 * @throws Exception
	 */
	private void fillAreaInColumnWithChar(int columnIndex, int rowBegin,
