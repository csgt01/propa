...
public class NonoSolver3 implements INonogramSolver {
   ...
   public char[][] getSolution() {
      ...
      try {
         ...
         solveRecursive();
         fillRowsWhenFullWithBlock();
         fillBlocksOnBeginningOfColumn();
   ...
   private char[][] fillBlocksOnBeginningOfColumn() throws Exception {
    ...
    * @throws Exception
    */
   private char[][] fillRowsWhenFullWithBlock() throws Exception {
            ...
            if (row.getBlocks() != null) {
               ...
               if (blockCounter == width) {
                  ...
                  for (Block block : row.getBlocks()) {
                     if (null != lastBlock) {
                        if (lastBlock.getColour().getName() == block.getColour().getName()) {
                           fillAreaInRowWithChar(getIndexOfRow(row), columnIndex, columnIndex + 1, '-');
                           columnIndex++;
                        }
                     }
                     boolean run = true;
                     ...
                     fillAreaInRowWithChar( getIndexOfRow(row), columnIndex, columnIndex + block.getHowMany(), block.getColour().getName());
...
//               System.out.println("Row:" + getIndexOfRow(row));
               // Wenn es noch keine Liste mit Möglichkeiten gibt wir sie hier
               // erstellt.
...
//               System.out.println("Pos1 size:" + possibilities.size() + " " + possibilities);
               // Unmögliche Möglichkeiten entfernen
               possibilities = erasePossibilitiesInRow(row, row.getPossibilities());
               row.setPossibilities(possibilities);
//               System.out.println("Pos2 size:" + possibilities.size() + " " + possibilities);
...
//               System.out.println("------------");
    ...
    * @param possibilitiy
    */
   private void fillListIntoMatrixInRow(int row, LinkedList<String> possibilitiy) {
