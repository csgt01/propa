...
public class NonoSolver3 implements INonogramSolver {
	 ...
	 * @return
	 */
	private boolean checkStateOfWrittenMatrixByRow(char[][] checkMatrix,
			int rowIn) {
...
//			for (Row row : getRows()) {
//				if (row.isGone()
//						&& !checkStateOfWrittenMatrixByRow(matrix,
//								getIndexOfRow(row))) {
...
//			}
	...
	private void fillBlocksOnEndOfColumn(Column column, int blockIndex,
		...
		if (rowInt > -1 && matrix[rowInt][indexOfColumn] != '*') {
			...
			if (blocks != null) {
				...
				if (matrix[rowInt][indexOfColumn] == colourBlock
						.getColorChar()) {
					...
					if (-1 < block - 1) {
						...
						if (nextBlock != null
							...
							fillAreaInColumnWithChar(indexOfColumn,
									rowInt, rowInt + 1, '-');
							rowInt--;
						}
						// nochmal aufrufen
	...
	private void fillBlocksOnEndOfRow(Row row, int blockIndex, int columnIndex)
		...
		if (columnInt > -1 && matrix[indexOfRow][columnInt] != '*') {
			...
			if (blocks != null) {
				Block colourBlock = blocks.get(block);
				if (matrix[indexOfRow][columnInt] == colourBlock
						.getColorChar()) {
...
//			}
	...
	private int getMinStartIndexOfBlock(ArrayList<Block> blocks,
		...
		for (int i = indexOfBlock - 1; i >= 0; i--) {
			...
			if (null != lastBlock
					&& lastBlock.getColorChar() == block.getColour()
							.getName()) {
				index++;
			}
			lastBlock = block;
