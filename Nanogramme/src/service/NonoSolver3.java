...
public class NonoSolver3 implements INonogramSolver {
      ...
      try {
         ...
         if (getStarCountInRiddle() > 0) {
            ...
            if (getPossibillitySizeOfColumn() > getPossibillitySizeOfRow()) {
               ...
               theMatrix = solveByBrutForceByRow(new ArrayList<ArrayList<String>>(), 0, 0);
            } else {
               System.out.println("vfdsfffdfsfdsdfsdfssdffdsfdsf2");
               possibillities = getPossibillitySizeOfColumn();
               theMatrix = solveByBrutForceByColumn(new ArrayList<ArrayList<String>>(), 0, 0);
            }
   ...
   private void solveRecursive() throws Exception {
      ...
      for (Column column : getColumns()) {
         ...
         if (possibilities == null || possibilities.size() == 0) {
            ...
            possibilities = getPossibilitiesForRowOrColumn(true, column.getBlocks(), firstConditionOfColumn, possibilities, 0, 0);
      ...
      while (run) {
         ...
         for (Row row : getRows()) {
            ...
            if (row.getBlocks().size() > 0 && !row.isGone()) {
               ...
               if (possibilities == null || possibilities.size() == 0) {
                  ...
                  possibilities = getPossibilitiesForRowOrColumn(false, row.getBlocks(), firstConditionOfRow, possibilities, 0, 0);
         ...
         for (Column column : getColumns()) {
            ...
            if (column.getBlocks().size() > 0 && !column.isGone()) {
               ...
               if (possibilities == null || possibilities.size() == 0) {
                  possibilities = getPossibilitiesForRowOrColumn(true, column.getBlocks(), firstConditionOfRow, possibilities, 0, 0);
                  possibilities.add(getFirstConditionOfColumn(column.getBlocks(), null, 0));
               }
               column.setPossibilities(possibilities);
               int posibillitiesBefore = possibilities.size();
    ...
    * @return
    */
   private ArrayList<LinkedList<String>> getPossibilitiesForRowOrColumn(boolean forColumn, LinkedList<Block> blocks1, LinkedList<String> aa, ArrayList<LinkedList<String>> possibilities,
    ...
    * in den Possibilities der Spalte hat.
    * 
    * @param possibility
    * @param rowInt
    * @return
   ...
   // TODO row setEntries mit char...
   /**
    * Überprüft, ob jede Reihe(ein char) in dieser Spalte mindestens eine Übereinstimmung
    * in den Possibilities der Reihe hat.
    * 
    * @param possibility
    * @param rowInt
    * @return
