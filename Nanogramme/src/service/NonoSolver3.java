...
//					 run1 = false;
	...
	private void fillWithEmptyAfterGone(Row row) throws Exception {
		...
		for (int blockInt = 0; blockInt < blocks.size(); blockInt++) {
			...
			if (block.isGone()) {
				if (blockInt == 0) {
					if (block.getColor().equals(
							blocks.get(blockInt + 1).getColor())
		...
		if (blocks != null && blocks.size() > 1) {
			for (int blockIndex = 0; blockIndex < blocks.size(); blockIndex++) {
				...
				if (block.isGone()) {
					...
					if (blockIndex == 0) {
						for (int newIndex = (blockIndex + 1); newIndex < blocks
								.size(); newIndex++) {
							...
							if (!checkBlock.isGone()) {
								if (checkBlock.getMinStartIndexNew() <= maxIndex) {
									if (block.getColor().equals(
											checkBlock.getColor())) {
										checkBlock
												.setMinStartIndexNew(maxIndex + 2);
									} else {
										checkBlock
												.setMinStartIndexNew(maxIndex + 1);
									}
								}
							}
						}
