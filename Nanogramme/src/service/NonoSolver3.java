...
public class NonoSolver3 implements INonogramSolver {
   ...
   private int solve() {
      try {
         ...
         while (run1) {
            ...
            if (starCount <= getStarCountInRiddle()) {
               System.out.println("QWQQWQWQWWWWWQQQQQQQ");
               // showMatrix();
               // showBlockGoneTrue();
      ...
      // System.out.println("OUTPUTPITJSDFADSFASF");
      // showBlockGoneTrue();
      // }
      catch (Exception e) {
         e.printStackTrace();
   ...
   private void fillWithEmptyAfterGone(Row row) throws Exception {
      ...
      for (int blockInt = 0; blockInt < blocks.size(); blockInt++) {
         ...
         if (block.isGone()) {
            if (blockInt == 0) {
               if (block.getColor().equals(blocks.get(blockInt + 1).getColor()) && block.getEndIndex() != null && (block.getEndIndex() + 1) < riddle.getWidth()) {
                  writeCharInMatrix(indexOfRow, '-', block.getEndIndex() + 1);
               }
            } else if (blockInt == blocks.size() - 1) {
               if (block.getColor().equals(blocks.get(blockInt - 1).getColor()) && block.getStartIndex() != null && (block.getStartIndex() - 1) > -1) {
   ...
   private void fillWithEmptyAfterGone(Column column) throws Exception {
      ...
      for (int blockInt = 0; blockInt < blocks.size(); blockInt++) {
         ...
         if (block.isGone()) {
            if (blockInt == 0) {
               if (block.getColor().equals(blocks.get(blockInt + 1).getColor()) && block.getEndIndex() != null && (block.getEndIndex() + 1) < riddle.getHeight()) {
            ...
            if (indeces.size() > 1) {
               for (int column = indeces.first() + 1; column < indeces.last(); column++) {
                  if (!indeces.contains(column)) {
                     ...
                     writeCharInMatrix(getIndexOfRow(row), block.getColor().charAt(0), column);
                  }
               }
            }
         }
                  ...
                  for (int newIndex = (blockIndex + 1); newIndex < blocks.size(); newIndex++) {
                     ...
                     if (!checkBlock.isGone()) {
                        if (checkBlock.getMinStartIndexNew() <= maxIndex) {
                           if (block.getColor().equals(checkBlock.getColor())) {
