		...
		for (int i = 0; i < root.getNodes().length; i++) {
			if (root.getNode(i) != null) {
				if (root.getNode(i).getReferencesOfChilds() != null) {
					if (returnNode.getReferencesOfChilds() == null
							|| (root.getNode(i).getReferencesOfChilds() < returnNode
									.getReferencesOfChilds() && root.getNode(i).getReferencesOfChilds() > 1)) {
						returnNode = root.getNode(i);
					}
				} else {
					Node node = findNodeWithLessChildsReferences(root
							.getNode(i));
					if ((node.getReferencesOfChilds() < returnNode
									.getReferencesOfChilds() && node.getReferencesOfChilds() > 1)) {
						returnNode = node;
					}
				}
			}
